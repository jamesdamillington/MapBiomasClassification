...
meat_areas %>% 
  filter(source == "Map") %>%
  ggplot(aes(x=Class, y=intensity, fill=Class)) + 
    geom_bar(stat="identity", colour="white", position = "dodge") +
...
meat_summary %>%
  #filter(source == "MB") %>%
  ggplot(aes(x=Class, y=int_mn, fill=Class)) + 
...
meat_summary %>%
  #filter(source == "Map") %>%
  ggplot(aes(x=Class, y=int_md, fill=Class)) + 
...
This time limit the y-axis to 1
```{r fig.width=8}
meat_summary %>%
  #filter(source == "MB") %>%
  ggplot(aes(x=Class, y=int_mn, fill=Class)) + 
